...
const { Performance, User, Calendar, AwayDay, MovieGenre } = db;
...
export default class PerformancesController {
  ...
  async feedback(ctx) {
    const { performanceId, rating } = ctx.request.body;
    ...
    if (performance[otherParticipant]) {
      ctx.throw(400, 'You already voted');
    }

    performance.updateAttributes({
  ...
  async updateOffer(ctx) {
    const { performanceId, status: newStatus } = ctx.request.body;
